...
class Str
{
     ...
     * @param int $length 显示宽度
     * @param string $pad 点位字符串
     * @param int $pos    占位的位置
     * @return string
     */
    public static function strpad($str, $length, $pad = ' ', $pos = STR_PAD_RIGHT)
    {
        if (!$str) return str_repeat($pad, $length);
        $str    = trim($str);
        $str1   = trim(strip_tags($str));
        $hasTag = $str1 != $str;
        $mblen  = mb_strlen($str1, 'UTF-8');
        $len    = strlen($str1);
        $length += ($len - $mblen) / 2;
        $result = str_pad($str1, $length, $pad, $pos);
        if ($hasTag) {
            $repeat = (strlen($result) - $len) / strlen($pad);
            $result = $str . str_repeat($pad, $repeat);
        }
        return $result;
    }
