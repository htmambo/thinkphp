...
class Rbac
{
    ...
    public static function getAccessList($authId)
    {
        ...
        $baseSql   = 'SELECT node.id,node.name FROM ' .
            $table['role'] . " AS role," .
            $table['user'] . " AS user," .
        ...
        $map = [
                'user.user_id' => $authId,
                'user.role_id' => ['exp', '=role.id'],
                'access.role_id=role.id OR (access.role_id=role.pid AND role.pid!=0)',
                'role.status' => 1,
                'access.node_id' => ['exp', '=node.id'],
                'node.level' => 1,
                'node.status' => 1
        ];
        $where = Db::parseWhere($map);
        $sql = $baseSql . $where;
        $apps   = $db->query($sql);
        $access = array();
        foreach ($apps as $app) {
    ...
    public static function getModuleAccessList($authId, $module)
    {
        ...
        $baseSql   = 'SELECT access.node_id FROM ' .
            $table['role'] . " AS role," .
            $table['user'] . " AS user," .
            $table['access'] . " AS access ";
        $where = Db::parseWhere($map);
