 ...
 * @method boolean set($name, $value, $expire = null)
 * @method boolean rm($name, $ttl = false)
 * @method boolean clear()
 */
class Cache
{
    ...
    public function connect($type = '', $options = array())
    {
        ...
        $class = strpos($type, '\\') ? $type : 'Think\\Driver\\Cache\\' . ucwords(strtolower($type));
    ...
    public function getOrSet($key, $callable, $expire = null)
    {
        $obj = static::getInstance();
        if (($value = $obj->get($key)) !== false) {
            return $value;
        }
        ...
        if (!$obj->set($key, $value, $expire)) {
            E('Failed to set cache value for key ' . json_encode($key) . ' and value is ' . $value);
        }
        return $value;
    }
