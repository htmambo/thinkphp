 ...
 * 数据库驱动基类
 * 
 * @method array getFields($tableName) 获取表字段信息
 * @method array|bool procedure($sql, $parse = false) 执行存储过程
    ...
    public function connect($config = '', $linkNum = 0, $autoConnection = false)
    {
        if (!isset($this->linkID[$linkNum])) {
            ...
            try {
                ...
                elseif ($config['debug']) {
                    throw new PDOException($e, $config, '');
//                    E($e->getMessage());
                }
            }
        }
    ...
    {
        return $this->_runPdo($str, $fetchSql, 'query', $master);
    }
    ...
    {
        return $this->_runPdo($str, $fetchSql, 'execute');
    }

    private function _runPdo($str, $fetchSql, $type, $master = false)
    {
        $isWrite = $type === 'execute';
        $this->initConnect($isWrite||$master);
        if (!$this->_linkID) {
            return false;
        }
        ...
        if(C('APP_DEBUG') && strpos($this->queryStr, 'SHOW COLUMNS FROM')===0) {
            //开启了调试模式的话，这种查询就不记了，因为调试模式会忽略字段缓存
        } else {
            if($isWrite) {
                $this->executeTimes++;
                N('db_write', 1); // 兼容代码
     ...
     * @throws PDOException
     */
    public function error(\PDOException $exception)
    {
        if($exception) {
            ...
            $exception = new \PDOException($error[2], $error[1]);
        ...
        throw new PDOException($exception, $this->config, $this->queryStr, $error['function']);
    }
    ...
    {
        $sql = ($model && isset($this->modelSql[$model])) ? $this->modelSql[$model] : $this->queryStr;
        if (!$sql) {
            $sql = $this->queryStr;
        }
        return $sql;
    ...
    protected function debug($start)
    {
        ...
        if ($this->config['debug']) {
            // 开启数据库调试模式
            if ($start) {
                $this->debug_begin = true;
                G('queryStartTime');
            }
            else if ($this->debug_begin) {
                $this->debug_begin = false;
                // 记录操作结束时间
                G('queryEndTime');
