...
class Cx extends TagLib
{
    ...
    public function _compare($tag, $content, $type = 'eq')
    {
        ...
        $parseStr = '<?php if(((' . $name . ') ' . $type . ' ' . $value;
        if ($type === ' & ' || $type === 'bitand') {
            $parseStr .= ')!==0';
        } else {
            $parseStr .= ')';
     ...
     * @return string|void
     */
    public function _can($tag, $content)
    {
        $name = $tag['name'];
        $items = explode(',', $tag['param']);
        $append = [];
        if ($items) {
        ...
        $parseStr = '<?php if(\Think\Auth::checkAuth(\'' . $name . '\'' . $append . ')): ?>';
        $parseStr .= $this->tpl->parse($content);
        $parseStr .= '<?php endif; ?>';
