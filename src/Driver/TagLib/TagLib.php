...
class TagLib
{
    ...
    public function __construct()
    {
        ...
        $this->tpl = \Think\Think::instance('Think\\Driver\\Template\\Think');
    }
        ...
        if (isset($xml['@attributes'])) {
            ...
            if ($array) {
                ...
                foreach ($attrs as $name) {
                    ...
                    } else if (false !== array_search($name, $must)) {
    ...
    public function autoBuildVar($name)
    {
        if ('Think.' == substr($name, 0, 6)) {
            ...
            //检查数组的key是否被'包围
            if (preg_match_all('@\[([^\]]+)\]@i', $name, $match)) {
                foreach ($match[1] as $k => $keyname) {
                    $firstChar = substr($keyname, 0, 1);
                    if (!is_numeric($keyname) && !in_array($firstChar, ['"', "'", '$'])) {
                        $name = str_replace($match[0][$k], "['" . $keyname . "']", $name);
                    }
                }
            }
            $name = '$' . $name;
