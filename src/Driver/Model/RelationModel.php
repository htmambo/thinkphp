...
class RelationModel extends Model
{
        ...
        if($name) {
            if(is_array($name)) {
                foreach($name as $tmp1) {
                    $tmp2 = parse_name($tmp1, 1);
                    $tabName[$tmp2] = $tmp1;
                }
            } else if (is_string($name)) {
                $tmp2 = parse_name($name, 1);
                $tabName[$tmp2] = $name;
            }
        }
        if (!empty($this->_link)) {
            foreach ($this->_link as $key => $val) {
                ...
                if (empty($name) || true === $name || $mappingName == $tabName || isset($tabName[$mappingClass])) {
                    ...
                    switch ($mappingType) {
                        case self::HAS_ONE:
                            ...
                            $relationData     = $model->where($mappingCondition)->field($mappingFields)->find(['cache' => 30]);
                        ...
                        case self::HAS_MANY:
                            ...
                            // 延时获取关联记录
                            $relationData = $model->where($mappingCondition)->field($mappingFields)->order($mappingOrder)->limit($mappingLimit)->select(['cache' => 30]);
                            if (!empty($val['relation_deep'])) {
                    ...
                    if (!$return) {
                        $keyName = $mappingName;
                        if(isset($tabName[$mappingName])) {
                            $keyName = $tabName[$mappingName];
                        }
                            ...
                            foreach ($fields as $field) {
                                if (strpos($field, ':')) {
                                    ...
                                    if (isset($result[$nick])) {
                                        $result[$keyName . '.' . $nick] = $relationData[$relationName];
                                    } else {
                                    ...
                                    if (isset($result[$field])) {
                                        $result[$keyName . '.' . $field] = $relationData[$field];
                                    } else {
    ...
    public function relation($name)
    {
        if (is_string($name) || is_array($name)) {
            ...
            if (is_string($name)) {
                if($name === '*') {
                    $name = array_keys($this->_link);
                } else {
                    $name = explode(',', $name);
                }
            }
            $link                  = array_merge($link, $name);
            $link = array_map(function($f) {
                return trim($f);
            }, $link);
            $this->options['link'] = array_unique($link);
        }
        else {
