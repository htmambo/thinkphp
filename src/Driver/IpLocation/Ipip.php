...
class Ipip
{
    ...
    {
        $this->database = CORE_PATH . 'Driver/IpLocation/data/ipipfree.ipdb';
        ...
        {
            throw new Exception("Error determining the size of \"{$this->database}\".");
        }
     ...
     * @throws Exception
     */
    private function finddata($ip, $language = 'CN')
    {
        if (is_resource($this->file) === FALSE)
        {
            throw new Exception('closed IPIP DB.');
        }
    ...
    {
        $array = $this->finddata($ip, $language);
        if (NULL == $array)
        {
            return NULL;
        }
        $fields = $this->meta['fields'];
        array_unshift($fields, 'ip');
        array_unshift($array, $ip);
        return array_combine($fields, $array);
    }
