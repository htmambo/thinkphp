...
/**
 * @property string|multitype $fontSize
 */
class Verify
{
    ...
    function _create() {
        // 图片宽(px)
        $this->imageW || $this->imageW  = intval($this->length * $this->fontSize * 2);
        // 图片高(px)
    ...
    function _getTTF() {
        // 验证码使用随机字体
        $ttfPath = CORE_PATH . 'Helper/Verify/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
    ...
    private function _background() {
        $gb = '';
        if ($this->useImgBg) {
            $path = CORE_PATH . 'Helper/Verify/bgs/';
            $dir  = dir($path);
        ...
        if ($this->useImgBg && $gb) {
            ...
            for ($i = 0; $i < $this->imageW; $i++) {
                $color = imagecolorallocate($this->_image, (int)$c[0], (int)$c[1], (int)$c[2]);
    ...
    function ttffont() {
        ...
        $text_color = imagecolorallocate($this->_image, (int)$this->_color[0], (int)$this->_color[1], (int)$this->_color[2]);
        for ($i = 0; $i < $this->length; $i++) {
            ...
            imagettftext($this->_image,
                ...
                $font[$i]['angle'],
                max($x, 1),
                intval($the_box["top"] + ($this->imageH / 2) - ($the_box["height"] / 2)),
                $text_color,
                $this->fontttf,
                $code[$i]);
            $x += $font[$i]['width'];
