...
class Dispatcher
{
    ...
    public static function dispatch()
    {
        ...
        if (empty($_SERVER['PATH_INFO'])) {
            ...
            // 去除URL后缀，是否需要强制判断URL确实是这个后缀？
            $_SERVER['PATH_INFO'] = preg_replace('/\.' . __EXT__ . '$/i', '', __INFO__);
            $paths                = explode($depr, trim($_SERVER['PATH_INFO'], $depr));
        }

        // URL常量
        define('__SELF__', isset($_SERVER[C('URL_REQUEST_URI')]) ? strip_tags($_SERVER[C('URL_REQUEST_URI')]) : '');
        ...
        if (MODULE_NAME && is_dir(APP_PATH . MODULE_NAME)) {
            ...
            if (APP_STATUS && is_file(MODULE_PATH . 'Conf/' . APP_STATUS . '.php')) {
                C(load_config(MODULE_PATH . 'Conf/' . APP_STATUS . '.php'));
            }

            // 加载模块别名定义
            if (is_file(MODULE_PATH . 'Conf/alias.php')) {
                Loader::addClassMap(include MODULE_PATH . 'Conf/alias.php');
            }
    ...
    private static function getController(&$paths, $urlCase)
    {
        if (defined('BIND_CONTROLLER')) {
            ...
            if ($paths) {
                ...
                //这里需要修复一下
                if (count($paths) >= 3 && $paths[0] == MODULE_NAME) {
                    array_shift($paths);
                }
    ...
    private static function getModule(&$paths)
    {
        ...
        if (in_array_case($module, C('MODULE_DENY_LIST'))) {
            // 禁止访问原始模块，必须使用BIND_MODULE来运行
            self::inValidAccess();
        }
    ...
    private static function inValidAccess()
    {
