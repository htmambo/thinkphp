 ...
 * @property-read string removeTrash
 */
class Upload
{
    /**
    ...
    {
        return call_user_func(array($this->uploader, $method), $args);
    }
    /**
     * 使用 $this->name 获取配置
     *
    ...
    public function __get(string $name)
    {
        ...
        $this->error = L('THE SPECIFIED PARAMETER DOES NOT EXIST!');
        return false;
    }
    /**
     * @throws Exception
    ...
    public function upload($files = '')
    {
        ...
        if (empty($files)) {
            $this->error = L('NO UPLOADED FILES!');
            return false;
        }
        /* 检测上传根目录 */
        ...
        foreach ($files as $key => $file) {
            ...
            if (in_array($ext, array('gif', 'jpg', 'jpeg', 'bmp', 'png', 'swf'))) {
                ...
                if (empty($imginfo) || 'gif' == $ext && empty($imginfo['bits'])) {
                    $this->error = L('ILLEGAL IMAGE FILES!');
                    continue;
                }
            }
            /* 保存文件 并记录保存成功的文件 */
    ...
    private function setDriver($driver = null, $config = null)
    {
        ...
        $class = strpos($driver, '\\') ? $driver : 'Think\Driver\Upload\\' . ucfirst(strtolower($driver));
        $this->uploader = new $class($config);
        if (!$this->uploader) {
            throw new ClassNotFoundException(L('_CLASS_NOT_EXIST_') . $driver);
        }
    }
    /**
     * 检查上传的文件
    ...
    private function check(array $file): bool
    {
        ...
        /* 无效上传 */
        if (empty($file['name'])) {
            $this->error = L('UNKNOWN UPLOAD ERROR!');
        }
        /* 检查临时文件是否存在 */
        if (!file_exists($file['tmp_name'])) {
            $this->error = L('TEMPORARY FILES HAVE BEEN LOST!');
            return false;
        }
        /* 检查是否合法上传 */
        if (!is_uploaded_file($file['tmp_name'])) {
            $this->error = L('ILLEGAL UPLOAD OF FILES!');
            return false;
        }
        /* 检查文件大小 */
        if (!$this->checkSize($file['size'])) {
            $this->error = L('THE UPLOADED FILE SIZE DOES NOT MATCH!');
            return false;
        }
        /* 检查文件Mime类型 */
        ...
        if (!$this->checkExt($file['ext'])) {
            $this->error = L('UPLOADING FILE SUFFIX IS NOT ALLOWED');
            return false;
        }
        /* 通过检测 */
        return true;
    ...
    private function error($errorNo)
    {
        switch ($errorNo) {
            case 1:
                $this->error = L('THE UPLOADED FILE EXCEEDS THE VALUE LIMIT OF THE UPLOAD_MAX_FILESIZE OPTION IN PHP.INI!');
                break;
            case 2:
                $this->error = L('THE UPLOADED FILE SIZE EXCEEDS THE VALUE SPECIFIED BY THE MAX_FILE_SIZE OPTION IN THE HTML FORM!');
                break;
            case 3:
                $this->error = L('ONLY PARTIAL FILES ARE UPLOADED!');
                break;
            case 4:
                $this->error = L('NO FILES HAVE BEEN UPLOADED!');
                break;
            case 6:
                $this->error = L('TEMPORARY FOLDER NOT FOUND!');
                break;
            case 7:
                $this->error = L('FILE WRITING FAILED!');
                break;
            default:
                $this->error = L('UNKNOWN UPLOAD ERROR!');
        }
    ...
    {
        return empty($this->config['mimes']) || in_array(strtolower($mime), $this->mimes);
    }
    /**
     * 检查上传的文件后缀是否合法
     * @param string $ext 后缀
    ...
    {
        return empty($this->config['exts']) || in_array(strtolower($ext), $this->exts);
    }
    /**
     * 根据上传文件命名规则取得保存文件名
    ...
    private function getSaveName($file)
    {
        ...
        if (empty($rule)) {
            ...
            if (empty($savename)) {
                $this->error = L('File naming rules are wrong!');
                return false;
            }
        }
        /* 文件保存后缀，支持强制更改文件后缀 */
