...
class Run extends Migrate
{
    ...
    protected function configure()
    {
        $this->setName('migrate:run')
            ...
            ->setDescription(L('Migrate the database'))
            ->addOption('--target', '-t', InputOption::VALUE_REQUIRED, L('The version number to migrate to'))
            ->addOption('--date', '-d', InputOption::VALUE_REQUIRED, L('The date to migrate to'))
            ->setHelp(<<<EOT
...
EOT
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $output->writeln('');
        $output->writeln('<comment>' . L('All Done. Took {$seconds}s.', ['seconds' => sprintf('%.4f', $end - $start)]) . '</comment>');
    }
    ...
    protected function migrate($version = null)
    {
        ...
        if (null === $version) {
            ...
            if (0 != $version && !isset($migrations[$version])) {
                $this->output->writeln(L('<comment>warning</comment> {$ver} is not a valid version', ['ver' => $version]));
                return;
            }
        }
