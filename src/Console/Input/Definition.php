...
class Definition
{
    ...
    public function addArgument(Argument $argument)
    {
        if (isset($this->arguments[$argument->getName()])) {
            throw new \LogicException(L('An argument with name "{$name}" already exists.', ['name' => $argument->getName()]));
        }

        if ($this->hasAnArrayArgument) {
            throw new \LogicException(L('Cannot add an argument after an array argument.'));
        }

        if ($argument->isRequired() && $this->hasOptional) {
            throw new \LogicException(L('Cannot add a required argument after an optional one.'));
        }
    ...
    public function addOption(Option $option)
    {
        if (isset($this->options[$option->getName()]) && !$option->equals($this->options[$option->getName()])) {
            throw new \LogicException(L('An option named "{$name}" already exists.', ['name' => $option->getName()]));
        }
        ...
        if ($option->getShortcut()) {
            foreach (explode('|', $option->getShortcut()) as $shortcut) {
                if (isset($this->shortcuts[$shortcut])
                    ...
                    throw new \LogicException(L('An option with shortcut "{$shortcut}" already exists.', ['shortcut' => $shortcut]));
                }
            }
        }
    ...
    {
        if (!isset($this->shortcuts[$shortcut])) {
            throw new \InvalidArgumentException(L('The "{$name}" option does not exist.', ['name' => '-' . $shortcut]));
        }
