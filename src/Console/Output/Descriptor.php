...
class Descriptor
{
    ...
    public function describe(Output $output, $object, array $options = [])
    {
        ...
        switch (true) {
            ...
            case $object instanceof Console:
                ...
                break;
            default:
                throw new \InvalidArgumentException(L('Object of type "{$type}" is not describable.', ['type' => get_class($object)]));
        }
    }
    ...
    protected function describeInputArgument(InputArgument $argument, array $options = [])
    {
        if (null !== $argument->getDefault()
            ...
            $default = L('<comment> [default: {$default}]</comment>', ['default' => $this->formatDefaultValue($argument->getDefault())]);
        }
        else {
            $default = '';
        }
        ...
        $desc         = $this->output->splitStringByWidth($desc, -$totalWidth - 4, false, $default);
        $desc = implode("\n", $desc);
        $this->writeText(
            sprintf(
                "  <info>%s</info>%s%s%s",
                $argument->getName(),
                str_repeat(' ', $spacingWidth), // + 17 = 2 spaces + <info> + </info> + 2 spaces
                // 支持说明信息的换行对齐（添加/u以识别汉字，别给汉字搞得拆分后变成乱码了）。这个替换似乎不应该加上<info>和</info>的宽度？
//                preg_replace('/\s*\R\s*/u', PHP_EOL . str_repeat(' ', $totalWidth + 17), $argument->getDescription()),
                preg_replace('/\s*\R\s*/u', "\n" . str_repeat(' ', $totalWidth + 4), $desc),
                $default
            ), $options);
    }
    ...
    protected function describeInputOption(InputOption $option, array $options = [])
    {
        if ($option->acceptValue() && null !== $option->getDefault()
            ...
            $default = L('<comment> [default: {$default}]</comment>', ['default' => $this->formatDefaultValue($option->getDefault())]);
        }
        else {
            $default = '';
        }
        ...
        $desc         = $this->output->splitStringByWidth($desc, -$totalWidth - 4, false, $default . ($option->isArray() ? '<comment> (' . L('multiple values allowed') . ')</comment>' : ''));
        ...
        $this->writeText(
            sprintf(
                ...
                preg_replace('/\s*\R\s*/u', "\n" . str_repeat(' ', $totalWidth + 4), $desc),
                $default,
                $option->isArray() ? '<comment> (' . L('multiple values allowed') . ')</comment>' : ''
            ), $options);
    }
    ...
    protected function describeCommand(Command $command, array $options = [])
    {
        ...
        $this->writeText('<comment>' . L('Usage') . '</comment>', $options);
    ...
    protected function describeConsole(Console $console, array $options = [])
    {
        ...
        if (isset($options['raw_text']) && $options['raw_text']) {
            ...
            $this->writeText(
                sprintf('<comment>%s</comment>', L('Usage')) . "\n",
                $options
            );
            $this->writeText(
                sprintf('  command [%s] [%s]', L('options'), L('arguments')) . "\n\n",
                $options
            );
            ...
            if ($describedNamespace) {
                $msg = L('Available commands for the "{$name}" namespace:', ['name' => $describedNamespace]);
            }
            else {
                $msg = L('Available commands');
            }
            $this->writeText('<comment>' . $msg . '</comment>', $options);
