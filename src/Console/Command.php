...
class Command
{
    ...
    public function __construct($name = null)
    {
        ...
        //非系统自带命令
        if (substr($class, 0, 6) !== 'Think\\') {
            list($moud, $cont) = explode('\\', $class);
            // 加载自建命令的语言包
            $langs = [C('LANG_SET'), C('DEFAULT_LANG')];
            $langs = array_change_key_case($langs, CASE_LOWER);
            $langs = array_unique($langs);
            foreach ($langs as $lang) {
                $langSetFile = LANG_PATH . $lang . '.' . strtolower($cont) . '.php';
        ...
        if (!$this->name) {
            throw new \LogicException(L('The command defined in "{$name}" cannot have an empty name.', ['name' => get_class($this)]));
        }
    }
    ...
    {
        throw new \LogicException(L('You must override the execute() method in the concrete command class.'));
    }
    ...
    public function run(Input $input, Output $output)
    {
        ...
        $this->output = $output;
        if (
            Output::VERBOSITY_VERBOSE <= $output->getVerbosity()
            &&
            ...
            $output->comment($this->getDescription());
        }
    ...
    {
        if (!is_callable($code)) {
            throw new \InvalidArgumentException(L('Invalid callable provided to Command::setCode.'));
        }

        if (PHP_VERSION_ID >= 70400 && $code instanceof \Closure) {
    ...
    {
        $this->help = PHP_EOL . $help . PHP_EOL;
    ...
    public function getProcessedHelp()
    {
        ...
        $info         = $this->getHelp();
        if ($info) {
            $info = str_replace($placeholders, $replacements, $this->getHelp());
        }
        return $info;
    }
    ...
    public function setAliases($aliases)
    {
        if (!is_array($aliases) && !$aliases instanceof \Traversable) {
            throw new \InvalidArgumentException(L('$aliases must be an array or an instance of \Traversable'));
        }
    ...
    {
        if (!preg_match('/^[^\:]++(\:[^\:]++)*$/', $name)) {
            throw new \InvalidArgumentException(L('Command name "{$name}" is invalid.', ['name' => $name]));
        }
    }
