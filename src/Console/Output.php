 ...
 * @method void writerArrayByStyle($message, $style)
 * @method array splitStringByWidth($string, $width, $pad = true, $other = '', $lineSpace = 0)
 */
class Output
{
    ...
    {
        $class = '\\Think\\Console\\Output\\Driver\\' . ucwords($driver);
    ...
    public function block($style, $message)
    {
        //是数组的话需要去匹配宽度
        if ($style == 'error' && is_string($message)) {
            $message = ['', trim($message), ''];
        }
        if (is_array($message)) {
            $this->handle->writerArrayByStyle($message, $style);
        }
        else {
    ...
    public function __call($method, $args)
    {
        ...
        if ($this->handle && method_exists($this->handle, $method)) {
            ...
            throw new Exception(L('Method ({$method}) not exists', ['method' => __CLASS__ . '->' . $method]));
        }
    }
