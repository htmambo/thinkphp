...
class Readability implements LoggerAwareInterface
{
    ...
    protected function grabArticle(?\DOMElement $page = null)
    {
        ...
        for ($nodeIndex = 0; $allElements->item($nodeIndex); ++$nodeIndex) {
            ...
            // 如果节点是pre或者code，并且有class="hljs", 则将class="hljs"替换为class="language-"，这样方便后续的markdown转换
            if (0 === strcasecmp($tagName, 'pre') || 0 === strcasecmp($tagName, 'code')) {
                if ($node->hasAttribute('class') && strpos($node->getAttribute('class'), 'hljs') !== false) {
                    $node->setAttribute('class', str_replace('hljs ', 'language-', $node->getAttribute('class')));
                }
            }
