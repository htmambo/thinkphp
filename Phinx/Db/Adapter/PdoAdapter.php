...
abstract class PdoAdapter extends AbstractAdapter implements DirectActionInterface
{
     ...
     * @return void
     */
    protected function verboseLog($message, $style = ''): void
    {
        if (
        ...
        $scopes  = $output->getTerminalDimensions();
        $width   = $scopes[0] ?: PHP_INT_MAX;
        if(mb_strwidth($message)>$width) {
            $lineSpace = 4;
            $lines   = $output->splitStringByWidth($message, $width, false, '', $lineSpace);
            $message = implode(PHP_EOL, $lines);
            $message = preg_replace('/\s*\R\s*/u', "\n" . str_repeat(' ', $lineSpace), $message);
        }
        $output->block($style, $message);
    }
